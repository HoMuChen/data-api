...
const getAll = async ({ table, page=1, size=10, db=dbConfig.db, host=dbConfig.host, port=dbConfig.port, dev=__DEV__ }) => {
  if(__DEV__) {
    console.log(`[Store] getAll --> ${db}.${table}@${host}:${port}`);
  }
...
const getNear = async ({ table, latitude, longitude , n=1, db=dbConfig.db, host=dbConfig.host, port=dbConfig.port, dev=__DEV__ }) => {
  if(__DEV__) {
    console.log(`[Store] getNear --> ${db}.${table}@${host}:${port} with lat: ${latitude}, lng: ${longitude}`);
  }
